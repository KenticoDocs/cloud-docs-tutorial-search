import { ItemRecordsCreator } from './itemRecordsCreator';
import {
    PlatformMarkStart,
    PlatformMarkEnd,
    InnerItemMarkStart,
...
const articleWithMultipleCallouts = {
    ...
    content: {
        name: 'Content',
        value:
            `${InnerItemMarkStart}Callout number 1${InnerItemMarkEnd}` +
...
describe('searchableArticleCreator', () => {
    const firstParagraph = {
        content:
            'We will start by running a React sample application on your machine and updating an article in the sample project.\nAfterward, we can continue doing this...',
        title: 'Tutorial',
        heading: '',
        codename: 'first_tutorial',
        order: 1,
    ...
    test('creates a correct single article chunk and sanitizes its content', () => {
        ...
        const actualResult = itemRecordsCreator.createItemRecords(shortArticle, shortArticle.content.value);
    ...
    test('splits a longer article into 2 chunks and sanitizes their content', () => {
        ...
        const actualResult = itemRecordsCreator.createItemRecords(longArticle, longArticle.content.value);
